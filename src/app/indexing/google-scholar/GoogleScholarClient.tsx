...
interface GoogleScholarClientProps {
  currentUser: HealixUser | null;
}

export default function GoogleScholarClient({ currentUser }: GoogleScholarClientProps) {
  const [profileId, setProfileId] = useState("");
  const [step, setStep] = useState<"input" | "searching" | "found" | "syncing" | "complete">("input");
  
  // Mock found author data
    ...
    <div className="max-w-4xl mx-auto px-6 py-12 relative z-10 text-slate-200">
      ...
      <div className="text-center space-y-4 mb-12">
        ...
        <p className="text-xs text-slate-400 max-w-xl mx-auto leading-relaxed">
          Connect your Google Scholar Author ID to pull your publications database, h-index, and live citations directly into your verifiable BioLabs profile page.
        </p>
      </div>
              ...
              <form onSubmit={handleSearch} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">
                    Enter Google Scholar Author ID or Profile URL
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="e.g. z1H1jAAAAAJ or https://scholar.google.com/citations?user=z1H1jAAAAAJ..."
                ...
                >
                  <span>Verify & Search Scholar Profile</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </form>

              <div className="border-t border-slate-900 pt-5 space-y-3">
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Where do I find my ID?</span>
                <p className="text-[10px] text-slate-400 leading-relaxed">
                  Navigate to your public Google Scholar Citations page. Copy the string of characters after the <code className="text-[#3B82F6] font-mono font-semibold">user=</code> parameter in the URL. For example, if your URL is <code className="text-slate-400 font-mono text-[9px]">https://scholar.google.com/citations?user=z1H1jAAAAAJ&hl=en</code>, your Author ID is <code className="text-emerald-400 font-mono font-semibold">z1H1jAAAAAJ</code>.
                </p>
              </div>
            </motion.div>
          )}
          ...
          {step === "searching" && (
            ...
            >
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-white">Crawling Google Scholar Database</h4>
                <p className="text-[10px] text-slate-500">Locating user profile and indexing verified publication bibliographies...</p>
              </div>
            </motion.div>
          )}
            ...
            >
              <div className="bg-[#070B13]/70 border border-slate-800 p-5 rounded-2xl space-y-4 relative">
                ...
                <div className="grid grid-cols-3 gap-4 border-t border-slate-900 pt-4 text-center">
                  <div>
                    <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Citations</span>
                    <span className="text-base font-extrabold text-white mt-1 block">{authorData.citations}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">h-index</span>
                    <span className="text-base font-extrabold text-white mt-1 block">{authorData.hIndex}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">i10-index</span>
                    <span className="text-base font-extrabold text-white mt-1 block">{authorData.i10Index}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-900 pt-5 mt-6">
                ...
                >
                  Sync Publications ({authorData.papersCount} papers)
                </button>
              </div>
            </motion.div>
          )}
          ...
          {step === "syncing" && (
            ...
            >
              ...
              <div className="space-y-1.5 w-full max-w-xs">
                <h4 className="text-sm font-bold text-white">Importing Publication Bibliographies</h4>
          ...
          {step === "complete" && (
            ...
            >
              ...
              <div className="space-y-2">
                <h4 className="text-base font-bold text-white">Google Scholar Linked Successfully!</h4>
                <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                  Your Scholar metrics and {authorData.papersCount} papers have been integrated into your researcher passport card.
                </p>
              </div>
              ...
              <div className="bg-[#070B13]/40 border border-slate-900/60 p-4 rounded-xl text-left w-full space-y-2.5">
                <span className="text-[8px] font-extrabold uppercase tracking-widest text-[#3B82F6] flex items-center space-x-1.5">
                  ...
                  <span>Synced Metrics Summary</span>
                </span>
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>Author Verified Name:</span>
                  <span className="font-semibold text-white">{authorData.name}</span>
                </div>
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>Citations Count:</span>
                  <span className="font-semibold text-white">{authorData.citations}</span>
                </div>
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>Imported Works:</span>
                  <span className="font-semibold text-emerald-400">{authorData.papersCount} Publications</span>
                </div>
              </div>
